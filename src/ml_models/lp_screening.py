...
constraints) to obtain instance-specific base-case flows, then use DC
LODF/ISF sensitivities to compute post-contingency margins and prune
constraints with large margins.
...
- Consistent DC sensitivities — LODF/ISF formulas match the benchmark's
  post-contingency approximation for a given dispatch.
- Fast — LP relaxation solves in seconds even for large grids.
- Combinable — can stack with WARM, HINTS, and masked LAZY variants for
  further speedup. The masked LAZY variant only checks the screened keep-set;
  it is not a full-event N-1 guarantee without an independent checker.

Modes
-----
- LPSCREEN-τ: Fixed threshold LP screening (simplest, no ML).
- LPSCREEN+LAZY: LP screening + lazy callback restricted to the screened
  keep-mask. This remains a heuristic screening mode for omitted events.
    ...
    for cont in scenario.contingencies or []:
        ...
        for gen in cont.units:
            ...
            if bidx == ref_1b or bidx not in col_by_bus_1b:
                beta_by_line = {line_l.name: 0.0 for line_l in lines}
            else:
                col = isf_csc.getcol(col_by_bus_1b[bidx])
                beta_by_line = {
                    line_by_row[row].name: float(value)
                    for row, value in zip(col.indices.tolist(), col.data.tolist())
                    if line_by_row.get(row) is not None and abs(float(value)) >= _ISF_TOL
                }
            for line_l in lines:
                if line_l.name not in beta_by_line:
                    continue
                beta = float(beta_by_line[line_l.name])
