...
class WarmStartProvider:
    ...
    New:
      - apply_warm_start_to_model(..., mode="repair") now supports:
          * "repair"      -> robust warm start with feasibility repair against current instance
          * "commit-only" -> set only binary u and derived v/w
          * "as-is"       -> prior behavior (copy values; may violate constraints)
    """
    ...
    def apply_warm_start_to_model(
        ...
        """
        Set Start on model vars using the warm JSON for 'instance_name'.

        mode:
          - "repair"      -> apply and repair to satisfy easy constraints:
                             • seg_cap: pseg <= amount * u
                             • reserve headroom: scale reserves to fit headroom
                             • PTDF equality: set line_flow consistent with injections
                             • base line limits: set overflow slacks to meet limits
                             • contingency limits: set shared slacks to cover max violation
                             • reserve requirement: set shortfall to cover unmet amount
          - "commit-only" -> set only u, v, w
          - "as-is"       -> raw application (may violate constraints)

        Returns: number of variable Start attributes set.
        """
        # Normalize mode
        ...
        fname = f"warm_{_sanitize_name(instance_name)}.json"
        fpath = (self.base_warm / fname).resolve()
        if not fpath.exists():
            alt = f"warm_{_sanitize_name(scenario.name)}.json"
            fpath = (self.base_warm / alt).resolve()
            if not fpath.exists():
                return 0
        ...
        # 2) Segment power — "repair" clips to per-segment capacity when on
        ...
        # 3) Reserves — scale per gen,t to satisfy shared headroom constraint
        ...
        if reserve_td is not None and scenario.reserves:
            ...
            # Shortfall to satisfy reserve requirement
                    ...
                    for t in range(T):
                        ...
                        for g in r.thermal_units:
                            try:
                                ...
                                # if Start not set due to missing var or earlier error
                                try:
        ...
        # 4) Network: set flows consistent with PTDF equality and set overflow slacks to cover limits
        if scenario.lines and line_flow_td is not None:
            try:
                ...
                # Build injections and flows
                # inj_by_t[bus_1b] -> float
                ...
                # 5) Contingency slacks: pick maximum violation across all contingencies
                if (
                    ...
                    for cont in scenario.contingencies:
                        ...
                        for gen in cont.units:
                            ...
                            # p_g,t from repaired warm start
                            for t in range(T):
                                pg = _p_total(gen, t)
                ...
                # If any failure during network repair, silently skip network part
                pass
