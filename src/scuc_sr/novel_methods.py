...
def _set_gurobi_params(
    ...
    *,
    ...
    model.setParam("NoRelHeurTime", max(0.0, float(no_rel_heur_time)))
...
def optimize_with_active_set(
    ...
    for it in range(1, int(config.max_rounds) + 1):
        ...
        if remaining <= 0.0:
            break
        no_rel_heur_time = float(config.no_rel_heur_time) if it == 1 else 0.0
        no_rel_heur_time = min(no_rel_heur_time, max(0.0, remaining))
        _set_gurobi_params(
            model,
        ...
        logger.info(
            "ActiveSet iter %d/%d start: remaining=%.1fs active_constraints=%d",
            it,
        ...
        logger.info(
            "ActiveSet iter %d/%d solve done: status=%s optimize_wall=%.1fs total=%.1fs",
            it,
            int(config.max_rounds),
            status,
        ...
        logger.info(
            "ActiveSet iter %d/%d checked events: max_violation=%.6g new_violated=%d",
            it,
            int(config.max_rounds),
            iter_max_viol,
            len(violated),
        ...
        if not violated:
            ...
            break
        if status == GRB.TIME_LIMIT or it >= int(config.max_rounds):
            break
        if float(config.time_limit) - (time.time() - t_start) <= 1.0:
            break
...
def solve_rolling_horizon(
    ...
    *,
    ...
    for w_idx, start in enumerate(starts):
        ...
        no_rel_heur_time = float(config.no_rel_heur_time) if w_idx == 0 else 0.0
        no_rel_heur_time = min(no_rel_heur_time, max(0.0, remaining))
        _set_gurobi_params(
            model,
            time_limit=max(1e-3, remaining),
