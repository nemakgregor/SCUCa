...
def restore_solution(scenario: UnitCommitmentScenario, model: Model) -> Dict[str, Any]:
    ...
    Returns a nested dict with:
      ...
      - network:
          lines: dict[line_name] with
            ...
            contingency_overflow_pos: List[float]
            contingency_overflow_neg: List[float]
    ...
    covp_vars = getattr(model, "contingency_overflow_pos", None)
    covn_vars = getattr(model, "contingency_overflow_neg", None)
    if lines:
        ...
        for line in lines:
            ...
            covp = [0.0 for _ in range(T)]
            covn = [0.0 for _ in range(T)]
            ...
            if covp_vars is not None:
                for t in range(T):
                    covp[t] = float(covp_vars[line.name, t].X)
            if covn_vars is not None:
                for t in range(T):
                    covn[t] = float(covn_vars[line.name, t].X)
            net_out["lines"][line.name] = {
                ...
                "overflow_pos": ovp,
                "overflow_neg": ovn,
                "contingency_overflow_pos": covp,
                "contingency_overflow_neg": covn,
                "penalty": penalty,
            }
