...
def add_constraints(
    ...
    has_line_mask = keep_masks is not None and "line" in keep_masks
    has_gen_mask = keep_masks is not None and "gen" in keep_masks
    keep_line_pairs = set(keep_masks.get("line", set())) if has_line_mask else set()
    keep_gen_pairs = set(keep_masks.get("gen", set())) if has_gen_mask else set()
                ...
                if bus_1b == ref_1b or bus_1b not in col_by_bus_1b:
                    ...
                    for line_l in lines:
                        if not _mon_allowed(line_l.name):
                            ...
                            continue
                        if has_gen_mask and (line_l.name, gen.name) not in keep_gen_pairs:
                            n_skipped_gen += 2 * len(T_list)
                            continue
