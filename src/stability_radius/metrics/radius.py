...
def _dot_checked(name: str, left: np.ndarray, right: np.ndarray) -> np.ndarray:
...
def orth_projector(n: int) -> np.ndarray:
    ...
    return I - np.outer(one, one) / float(n)
...
def make_balance_map(
    ...
    if mode == "slack":
        ...
        e[slack_idx] = 1.0
        return I - np.outer(e, one.reshape(-1))
    ...
    if mode in ("custom", "agc"):
        ...
        return I - np.outer(a.reshape(-1), one.reshape(-1)) / s
...
def minimal_norm_radius_l2_balanced(
    ...
    """
    Gbal = H_full - np.mean(H_full, axis=1, keepdims=True)
...
def minimal_norm_radius_l2_preimage(
    ...
    """
    Gmat = _dot_checked("pre-image L2 sensitivity", H_full, P)  # (m, n)
...
def minimal_norm_radius_metric(
    ...
    M_inv = la.pinvh(M)
    Gmat = _dot_checked("metric-weighted sensitivity", H_full, P)  # (m, n)
...
def probabilistic_sigma_radius(
    ...
    Gmat = _dot_checked("sigma sensitivity", H_full, P)
...
def predict_overload_probability(
    ...
    Gmat = _dot_checked("overload sensitivity", H_full, P)  # (m, n)
    var = np.einsum("ij,jk,ik->i", Gmat, Sigma, Gmat, optimize=True)
