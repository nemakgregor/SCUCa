...
$RunGnnPr    = if ($env:RUN_GNN_PR)  { $env:RUN_GNN_PR  } else { "0" }
...
if ($RunGnnPr -eq "1") {
    ...
    & $PythonBin paper\scripts\eval_gnn_precision_recall.py `
...
if ($RunFixedK -eq "1") {
    ...
    foreach ($K in 64, 128, 256) {
        & $PythonBin -m src.paper.experiments `
...
if ($RunGrbtune -eq "1") {
    ...
    & $PythonBin paper\scripts\run_grbtune.py --case matpower/case118 --tune-time 1800 --solve-time 600
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $PythonBin paper\scripts\run_grbtune.py --case matpower/case300 --tune-time 3600 --solve-time 600
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host ""
}
