...
\end{itemize}
Thus $u_{g,t},v_{g,t},w_{g,t}\in\{0,1\}$, while segment outputs, reserves, reserve shortfalls, and overflow slacks are non-negative. The flow variables $f_{\ell,t}$ and total generation variables $p_{g,t}$ are continuous.
...
\end{align}
For early periods where the summation window reaches before $t=0$, initial commitment and residual up/down times from the instance data determine the fixed carry-over terms.
...
\label{eq:cont-gen}
\end{equation}
This expression assumes that the lost generation is balanced by the reference bus used in the PTDF calculation. Under distributed balancing, the term would instead include participation factors for the balancing generators.
...
\end{align}
The terms in \eqref{eq:obj} are defined as follows. The first line represents production costs, where $C^{\min}_{g,t}$ is the no-load cost and $C^{\mathrm{seg}}_{g,s,t}$ is the marginal cost for energy in each segment. The second term accounts for startup costs, $C^{\mathrm{su}}_g$. The remaining terms are high-cost penalties for violating soft constraints: $\pi^{\mathrm{res}}_k$ penalizes shortfalls in reserve requirements, while $\pi^{\mathrm{flow}}_{\ell,t}$ penalizes base-case and post-contingency transmission overflows. The multiplier $\gamma>1$ is used to prioritize resolving contingency violations. Because these overflow slacks can make the mathematical model feasible even when a physical line limit is exceeded, physical N-1 feasibility is determined only by the post-solve audit with zero raw violations, not by slack-satisfied model constraints alone.
...
\end{equation}
Contingency-line pairs where $s_{\ell}^m \ge \tau$ are pruned from the model for all time periods. Thus $\tau$ controls conservatism: smaller values prune more contingency pairs, whereas larger values retain more post-contingency constraints. This 1-NN screener is implemented using SciPy.
...
\end{equation}
If any calculated flow $f^{m}_{\ell,t}$ violates its corresponding emergency limit $F^{\mathrm{E}}_{\ell,t}$, one or more linear inequalities of the form~\eqref{eq:cont-line} are generated on demand and added to the MILP model as lazy constraints. Gurobi's \texttt{LazyConstraints=1} parameter is enabled, as required when lazy constraints are added through callbacks~\cite{GurobiLazy}. The callback checks integer incumbents, and the full LAZY variant adds all detected line- and generator-outage violations before accepting an incumbent. This keeps the root LP compact while still requiring accepted incumbents to pass the same N-1 check used in the audit. In this study we apply lazy enforcement only to the post-contingency constraints~\eqref{eq:cont-line}--\eqref{eq:cont-gen}, because they are by far the most numerous and their threshold-style slack penalties in the objective weaken the dual bound. The base-case thermal limits~\eqref{eq:base-lims} are retained in the root LP, since they are comparatively few ($|\mathcal{L}|\cdot T$) and their dual impact is bounded; a preliminary experiment in which~\eqref{eq:base-lims} were additionally made lazy did not yield consistent improvement and is therefore not reported here.
...
\end{align}
This configuration provides an initial guess (Start), a hint value (VarHintVal), and prioritizes branching on variables corresponding to earlier time periods. The GBDT classifier is implemented using CatBoost~\cite{Prokhorenkova2018}, configured with 500 iterations, a learning rate of 0.06, and a tree depth of 6.
...
\begin{figure*}
  ...
  \caption{Mean speed-up by method and case.}
...
\begin{figure}
  ...
  \caption{Constraint count and speed-up.}
  \label{fig:constraint_reduction_speedup}
\end{figure}

Fig.~\ref{fig:speedup_heatmap} summarizes mean speed-up by method and case. Detailed confidence intervals are kept in the released CSV artifacts. Fig.~\ref{fig:constraint_reduction_speedup} compares final model size and speed-up. Lazy variants start from a smaller root model and add violated N-1 constraints on demand. PRUNE variants remove screened post-contingency constraints before the solve.

All computational experiments were conducted using the Gurobi 12.1 solver on a machine equipped with an AMD Ryzen 7840HS processor and 32 GB of RAM. The benchmark suite consisted of six MATPOWER cases from the UnitCommitment repository~\cite{Xavier2024,MATPOWER,PEGASE}, which were used to formulate a multi-period DC-SCUC with full N-1 security constraints (cases 14, 30, 57, 89pegase, 118 and 300). To separate learning-driven gains from solver-parameter effects, we also ran a representative grbtune sanity check on RAW instances from case14, case118, and case300. The tuned parameter set was then evaluated against the same default RAW solve.

Learning components use fixed train, validation, and test pools. These pools are generated per benchmark case from dated UnitCommitment instances and then expanded by mode, so the number of solver runs is larger than the number of unique calendar instances. Only held-out test instances are used for reported speedups. The speedups were measured relative to a baseline RAW mode, which represented a complete MILP optimization solved without acceleration techniques. Performance was analyzed using the empirical cumulative distribution function (ECDF) of per-instance speedups. In the revision analysis we additionally computed bootstrap 95\% confidence intervals for the main speedup summaries and paired Wilcoxon signed-rank tests against the RAW baseline; the full CI table is released with the paper artifacts.


We performed 8,845 completed test solver runs over 360 test instances. These runs totaled 150.0 wall-clock hours and 94.3 solver-runtime hours. Of these runs, 93.9\% ended with an optimal solution. Another 4.6\% were infeasible, and 1.4\% hit the time limit. The training and preparation stage required 434 additional solver runs over 288 training instances. This added 62.8 wall-clock hours and 51.9 solver-runtime hours. Reported online speedups exclude this training and preparation time and assume that it is amortized over repeated solves from the same benchmark distribution.

At the instance level, 95.6\% of unique test instances were solved optimally at least once by one of the tested modes. In the main six-case benchmark, cases 14--89pegase used a 60-second limit and larger cases used 600 seconds. The training and validation sets were solved separately.


The robustness audit reports the percentage of instances solved optimally, found infeasible, timed out, or failed the post-solve audit for each mode in the released table artifacts. Full LAZY achieved perfect or near-perfect optimality rates across the main benchmark cases, while reduced variants are interpreted through the audit rather than through reduced-model status alone.

In contrast, the baseline methods are less robust on larger systems. For example, the RAW+COMMIT variant only solves 71\% of case300 instances optimally within the given time limit, compared to the perfect or near-perfect rates achieved by other methods. For case57, the repeated 77/23/0 pattern is not caused by the acceleration mechanisms: the same 14 dates are reported infeasible by all 23 tested modes, which points to structural infeasibility of those benchmark instances (insufficient reserve after the worst-case contingency) rather than to a screening or warm-start failure. These 14 dates are excluded from the speedup statistics reported below, so they cannot inflate or deflate the reported acceleration factors. In addition, the most aggressive pruning settings can reduce robustness on case300, which indicates that screening thresholds must be chosen by audited feasibility, not by reduced-model runtime alone.

Accordingly, PRUNE-0.10 is the recommended screening default in the revised study and is empirically safe on this benchmark after the post-solve audit.

\input{tables/main_results_summary.tex}

In Tab.~\ref{tab:main_results}, mean speedup is averaged over the cases where each mode was evaluated. Fixed-$K$ and grbtune rows are sanity checks. They cover fewer cases than the common comparison modes.

\input{tables/audit_pass_rate.tex}

Tab.~\ref{tab:audit_pass_rate} separates solver status from the post-solve audit. The ``no incumbent'' column includes structurally infeasible benchmark dates for which no physical schedule is available. Fixed-$K$ lazy variants can solve the reduced callback model while failing the audit, so their speedups are interpreted only on audited feasible runs.

The speedup factors relative to the RAW baseline are summarized in Tab.~\ref{tab:main_results} and Fig.~\ref{fig:speedup_heatmap}. The strongest average accelerations are delivered by empirically safe screening and by lazy enforcement. The WARM+PRUNE-0.10 configuration achieves the highest average speedup of 9.64x across the six benchmark cases. Among the lazy enforcement strategies, WARM+LAZY+BANDIT attains an average speedup of 8.61x, although its advantage over plain WARM+LAZY is marginal (mean ratio of per-instance speedups $\approx 1.00$). The results show that the best method can be case-dependent: pruning excels on case89pegase (mean speedup $25.83$x, 95\% CI $[24.40, 27.26]$), while lazy enforcement dominates case300 (mean speedup $17.38$x, 95\% CI $[15.39, 19.39]$). Fig.~\ref{fig:constraint_reduction_speedup} shows that lazy and pruning methods both reduce the root model size relative to RAW, while warm starts alone leave the model size essentially unchanged. Conversely, methods that rely only on warm starts and hints without effective constraint management offer little to no acceleration, underscoring that managing the complexity of N-1 security is the primary driver of performance.

\input{tables/constraint_management_summary.tex}

Tab.~\ref{tab:constraint_management} reports root rows relative to RAW. It separates the two mechanisms quantitatively. PRUNE-0.10 removes almost all explicit post-contingency constraints before optimization. Lazy modes start from the root model and add cuts later through callbacks; the lazy-cut column reports the average number of cuts added per run.

\input{tables/ablation_sanity_summary.tex}

The compact ablation summary in Tab.~\ref{tab:ablation_sanity} clarifies the negative and sanity-check results. The GNN screener produces speedups close to the 1-NN variant, with mean ratios from 0.987 to 1.035 across thresholds, but it does not yield a consistent downstream gain. The fixed-$K$ lazy-cut ablation shows that aggressive cut budgets can be fast on case118, yet on case300 all fixed-$K$ settings solve more slowly than plain LAZY/BANDIT and pass the strict violation audit in only 66\% of runs. Finally, Gurobi's built-in parameter tuning gives only 0.93x on a representative case118 instance and 1.02x on case300, so it does not explain the order-of-magnitude improvements from constraint management.
...
\subsection{Large-Scale Stress Test: case1354pegase}
\label{sec:case1354}

To probe scalability beyond the main six-case benchmark, we used case1354pegase (1{,}354 buses, 260 generators, 1{,}991 branches, 1{,}288 N-1 contingencies, 36 time steps) from MATPOWER/PEGASE~\cite{MATPOWER,PEGASE} as a feasibility stress test. MATPOWER describes these data as fictitious and intended for algorithm validation rather than operation or planning. The goal was to check whether the lazy callback design can run on a much larger grid. It was not meant to establish robustness or generalization at this scale. The full model without contingency constraints already contains 317{,}484 rows. Static N-1 enforcement would add $52{,}218{,}216$ candidate post-contingency rows in this implementation: this counts two-sided emergency limits for monitored line-contingency and generator-contingency pairs after excluding invalid outage/self-monitoring pairs and out-of-service elements across the 36 time steps. All runs therefore use the lazy-enforcement framework. Tab.~\ref{tab:case1354} and Fig.~\ref{fig:case1354_runtime} report one representative held-out instance. The larger case1354pegase campaign also included 24 training days and two held-out test days. This produced 74 training/preparation runs and 66 test-mode runs.

\input{tables/case1354_results.tex}

\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figures/case1354_runtime.png}
\caption{case1354pegase runtime.}
\label{fig:case1354_runtime}
\end{figure}

In Fig.~\ref{fig:case1354_runtime}, green bars mark N-1-feasible solutions. Red hatched bars mark solutions with post-contingency audit violations. Gray bars mark runs without a feasible integer solution within 600\,s. The annotations show the final MIP gap.

The WARM reference run fails to find a feasible integer solution within the budget, so no post-solve N-1 audit can be applied to that row.

The full lazy method (LAZY-All, $K{=}0$) finds an N-1-feasible solution with a 12.3\% MIP gap. Adding ML-guided warm starts helps: LAZY+COMMIT (1-NN commitment hints) achieves the best feasible objective at 8.2\% gap, and LAZY+GRU (GRU dispatch initialiser) reaches 11.4\% gap---both producing verified N-1-feasible solutions within the time limit.

For fixed-$K$ variants, OPT in Tab.~\ref{tab:case1354} means optimality of the reduced callback model. Full N-1 validity is decided by the post-solve audit. The fixed-$K$ variants (LAZY-K64, LAZY-BANDIT) terminate optimally and faster, but at the cost of N-1 feasibility. They report lower objective values because some violated contingency constraints were never enforced by the callback. LAZY-K128 finds no solution within the budget. This suggests that an intermediate cut budget can be worse than either extreme.
...
\begin{itemize}
    \item \emph{Scale.} The main benchmark cases (14--300 buses) are smaller than production-scale ISO networks (typically $10^4$--$6\!\cdot\!10^4$ buses). The supplementary case1354pegase stress test (Sec.~\ref{sec:case1354}) checks feasibility at 1{,}354 buses. It is not a robustness study at that scale. Further scaling to realistic grid sizes remains future work.
    \item \emph{Screening reliability.} The 1-NN screener relies on a database of solved instances, whose coverage of operating regimes determines the screening quality. No formal probabilistic guarantee on the fraction of missed binding contingencies is provided; robustness is verified empirically through the final feasibility and violation audits.
    ...
    \item \emph{Data splits.} Learning components are trained on fixed train, validation, and test pools with a fixed seed. Cross-validation of the resulting speedups is left for future work; the reported Wilcoxon tests concern per-instance differences, not split-level stability.
    \item \emph{Solver tuning.} We performed a representative grbtune sanity check on RAW instances from case14, case118, and case300 (Tab.~\ref{tab:ablation_sanity}). The tuned baseline was essentially unchanged on the tiny case14 instance, slightly slower on case118 (0.93x), and only 1.02x faster on case300, far below the gains from constraint screening or lazy enforcement. This does not replace a full tuning study over every instance, but it suggests that the reported gains are not explained by parameter tuning alone.
    \item \emph{Negative results.} The GNN screener, the GRU initializer, and the BANDIT lazy-cut budget are reported here as ablations that did not reliably outperform the simpler baselines; they should be interpreted as negative or inconclusive results in the present benchmark rather than as recommended techniques.
\end{itemize}
...
This study shows that constraint management is the most effective accelerator among the tested methods. The key is to reduce or delay post-contingency constraints while keeping exact feasibility checks. We tested 360 MATPOWER instances and 8,845 completed runs with Gurobi 12.1 on a Ryzen 7840HS/32GB system. Two strategies gave the largest and most consistent gains. The first is screening redundant contingencies before solving. It uses simple data-driven predictors and LODF/PTDF sensitivities. The second is generating only the necessary contingency constraints on demand during branch-and-bound.

The recommended screening threshold preserved audited feasibility in the reported benchmark runs, and lazy generation often achieved the best performance on the hardest individual instances. In aggregate, the strongest screening threshold produced the highest average speedup (approximately $9.6\times$ over a fully explicit baseline), while lazy generation dominated the largest case in the benchmark. The revision analysis also shows that these gains coincide with very large reductions in explicit contingency constraints, typically above 99.7\% on the larger systems, and are statistically significant on every benchmark case (paired Wilcoxon $p<10^{-7}$).
...
\bibitem{Xavier2019} \'A.~S. Xavier, F.~Qiu, and S.~Ahmed, ``Learning to solve large-scale security-constrained unit commitment problems,'' \emph{INFORMS J. Comput.}, vol.~33, no.~2, pp.~739--756, May 2021, doi: 10.1287/ijoc.2020.0976.
