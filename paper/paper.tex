...
Tab.~\ref{tab:constraint_management} separates the two mechanisms quantitatively: PRUNE-0.10 removes almost all explicit post-contingency constraints before optimization, whereas LAZY+BANDIT adds only the violated constraints that are discovered during callbacks.
...
\begin{itemize}
    ...
    \item \emph{Solver tuning.} We performed a representative \texttt{grbtune} sanity check on RAW instances from \texttt{case14}, \texttt{case118}, and \texttt{case300} (Tab.~\ref{tab:ablation_sanity}). The tuned baseline was essentially unchanged on the tiny case14 instance, slightly slower on \texttt{case118} (0.93x), and only 1.02x faster on \texttt{case300}, far below the gains from constraint screening or lazy enforcement. This does not replace a full tuning study over every instance, but it suggests that the reported gains are not explained by parameter tuning alone.
